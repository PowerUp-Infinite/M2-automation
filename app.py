#!/usr/bin/env python3
"""
PowerUp Infinite - Personalized Strategy Deck Generator
Generates a personalized .pptx deck for each customer from data files.
"""
...
BASE_DECK  = os.path.join(SCRIPT_DIR, 'Base Deck - M2 automation.pptx')

DATA_FILES = {
    'pf_level':       os.path.join(SCRIPT_DIR, 'PF_level.csv'),
    'riskgroup':      os.path.join(SCRIPT_DIR, 'Riskgroup_level.csv'),
    'scheme':         os.path.join(SCRIPT_DIR, 'Scheme_level.csv'),
    'categorization': os.path.join(SCRIPT_DIR, 'Scheme_Category_Catgorization.xlsx'),
    'questionnaire':  os.path.join(SCRIPT_DIR, 'PowerUp Questionnaire Responses Sample.xlsx'),
    'lines':          os.path.join(SCRIPT_DIR, 'Lines.csv'),
    'results':        os.path.join(SCRIPT_DIR, 'Results.csv'),
    'invested':       os.path.join(SCRIPT_DIR, 'Invested_Value_Line.csv'),
}

RATING_IMAGES = {
    'IN_FORM':     os.path.join(SCRIPT_DIR, 'InForm.png'),
    'ON_TRACK':    os.path.join(SCRIPT_DIR, 'OnTrack.png'),
    'OUT_OF_FORM': os.path.join(SCRIPT_DIR, 'OutofForm.png'),
    'OFF_TRACK':   os.path.join(SCRIPT_DIR, 'OffTrack.png'),
}
...
def load_data():
    data = {}
    data['pf_level']       = pd.read_csv(DATA_FILES['pf_level'])
    data['riskgroup']      = pd.read_csv(DATA_FILES['riskgroup'])
    data['scheme']         = pd.read_csv(DATA_FILES['scheme'])
    data['categorization'] = pd.read_excel(DATA_FILES['categorization'])
    data['questionnaire']  = pd.read_excel(DATA_FILES['questionnaire'])
    data['lines']          = pd.read_csv(DATA_FILES['lines'])
    data['results']        = pd.read_csv(DATA_FILES['results'])
    data['invested']       = pd.read_csv(DATA_FILES['invested'])
        ...
        if not has_rating:
            for x, sh in sorted(cells.items()):
                if abs(x - X_RATING) < 60_000:
                    ...
                    break
        if has_rating:
            img_path = RATING_IMAGES[str(rating)]
...
RISK_REWARD_DECK = os.path.join(SCRIPT_DIR, 'Risk_Reward_Slides_Automation.pptx')
...
def do_risk_reward_slides(prs, risk_profile, goals=None):
    ...
    """
    if not os.path.exists(RISK_REWARD_DECK):
        print("  Risk Reward: deck not found — skipping"); return

    rr_prs = Presentation(RISK_REWARD_DECK)
...
def generate_deck(pf_id, customer_name):
    print(f"\n{'='*60}")
    ...
    if pf_row.empty:
        print(f"ERROR: PF_ID '{pf_id}' not found. Available IDs:")
        for pid in pf_df['PF_ID']:
            print(f"  {pid}")
        return None
    pf_row = pf_row.iloc[0]

    # Match questionnaire row — PF_ID first, then name
    ...
    if q_row.empty:
        for _, r in qdf.iterrows():
            if str(r.get('Name', '')).lower().strip() == customer_name.lower().strip():
                q_row = r
                print(f"  Questionnaire: matched by exact name '{r['Name']}'")
                break

    if q_row.empty:
        first = customer_name.lower().split()[0]
        for _, r in qdf.iterrows():
            if first in str(r.get('Name', '')).lower():
                q_row = r
                print(f"  Questionnaire: partial name match '{r['Name']}'")
                break
    ...
    out_path = os.path.join(SCRIPT_DIR, f'{safe}_{pf_id[:12]}_deck.pptx')
    shutil.copy2(BASE_DECK, out_path)
    prs = Presentation(out_path)
    print(f"  Copied base deck -> {os.path.basename(out_path)}")
...
if __name__ == '__main__':
    if len(sys.argv) >= 3:
        _id   = sys.argv[1]
        _name = ' '.join(sys.argv[2:])
    else:
        print("PowerUp Infinite - Deck Generator\n" + "=" * 40)
        _id   = input("PF_ID         : ").strip()
        _name = input("Customer Name : ").strip()
    generate_deck(_id, _name)
